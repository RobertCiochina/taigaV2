...
/// Updates the local list row, then optionally uploads to the active service.
/// `context_widget` is the parent for confirmation dialogs.
void commitListEntryLocalAndMaybeRemote(const ListEntry& entry, QWidget* context_widget = nullptr);

/// "Series complete" prompt: if watched_episodes >= known total (and total > 0), and the
